...
async function start() {
  ...
  async function runMultiAuto(limit = 'nolimit') {
    ...
    const POOL_SIZE = Math.min(7,os.cpus().length - 4); // 1 for OS and 1 for this process, and 2 spare
    const runner = [];
    let notifyComplete;
    let running = 0;
    let test = 0;
    for( const config of allConfigs ) {
      ...
      runner.push(() => {
        ...
        exec(`npm test ufo auto ${limit} no no-progress ${outPath}`, (err, stdout, stderr) => {
          ...
          if ( stderr ) {
            console.warn(outPath, stderr);
          }
          const result = ((err || stdout) + stderr).toString();
