...
runAll();
  ...
  function runAll() {
    ...
    fs.writeFileSync(path.resolve('dicts', 'dict.json'), JSON.stringify([...State.dict.entries()]));
  }
  ...
  // not needed for now
    ...
    function runIteration1_old(Text, useRun = false, name) {
      const Ent = [];
      const text = Text || `
         In Congress, July 4, 1776
      ...
      console.log(name, Ent.map(({entropy}) => entropy));
      return dict;
    }
    ...
    function runIteration2(name) {
      ...
      let factors, dict;
      for( let i = 0; i < 30; i++ ) {
        ({dict,factors} = index(text.slice(i)));  
        ...
        fs.writeFileSync(
          path.resolve('output', `${approxEnt}.txt`), 
          factors.map(f => JSON.stringify(f)).join('\n')
        );
      }
